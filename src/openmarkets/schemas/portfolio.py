...
class PortfolioRiskMetrics(BaseModel):
    ...
    annualized_return_percent: float = Field(..., description="Geometrically annualized compound return (%)")
    annualized_volatility_percent: float = Field(..., description="Annualized standard deviation / volatility (%)")
    sharpe_ratio: float | None = Field(None, description="Sharpe ratio when volatility is non-zero")
...
class RollingBetaSeries(BaseModel):
    ...
    current_beta: float | None = Field(None, description="Most recent rolling beta value when estimable")
