...
class YFinanceFinancialsRepository(IFinancialsRepository):
    ...
    @staticmethod
    def get_ttm_income_statement(ticker: str) -> list[TTMIncomeStatementEntry]:
    ...
    @staticmethod
    def get_ttm_cash_flow_statement(ticker: str) -> list[TTMCashFlowStatementEntry]:
    ...
    @staticmethod
    def get_financial_calendar(ticker: str) -> FinancialCalendar:
        data = yf.Ticker(ticker).get_calendar()
        return FinancialCalendar(**data)

    @staticmethod
    def get_sec_filings(ticker: str) -> list[SecFilingRecord]:
        data = yf.Ticker(ticker).get_sec_filings()
        return [SecFilingRecord(**filing) for filing in data]

    @staticmethod
    def get_eps_history(ticker: str) -> list[EPSHistoryEntry]:
