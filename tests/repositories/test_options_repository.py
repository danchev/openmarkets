...
def test_get_option_expiration_dates(monkeypatch):
    ...
    res = repo.get_option_expiration_dates("AAPL")
    assert isinstance(res, list)
    assert all(isinstance(x, OptionExpirationDate) for x in res)


def test_get_call_put_options_empty(monkeypatch):
    ...
    repo = YFinanceOptionsRepository()
    assert repo.get_call_options("AAPL") is None
    assert repo.get_put_options("AAPL") is None


def test_get_call_put_options_nonempty(monkeypatch):
    calls = FakeDF(
        [
            {
                "contractSymbol": "C1",
    ...
    repo = YFinanceOptionsRepository()
    calls_res = repo.get_call_options("AAPL")
    puts_res = repo.get_put_options("AAPL")
    assert isinstance(calls_res, list) and calls_res[0].strike == 100
    assert isinstance(puts_res, list) and puts_res[0].strike == 90


def test_get_options_volume_analysis_no_expirations(monkeypatch):
    monkeypatch.setattr(
        "openmarkets.repositories.options.yf",
        type("Y", (), {"Ticker": lambda t, session=None: DummyTicker(options=[])}),
    )
    repo = YFinanceOptionsRepository()
    assert repo.get_options_volume_analysis("AAPL") == {"error": "No options data available"}


def test_get_options_volume_analysis_with_data(monkeypatch):
    ...
    repo = YFinanceOptionsRepository()
    out = repo.get_options_volume_analysis("AAPL")
    assert out["total_call_volume"] == 10
    assert out["total_put_volume"] == 20
    assert out["put_call_ratio_volume"] == pytest.approx(20 / 10)


def test_get_options_by_moneyness_errors(monkeypatch):
    # no currentPrice
    monkeypatch.setattr(
        "openmarkets.repositories.options.yf", type("Y", (), {"Ticker": lambda t, session=None: DummyTicker(info={})})
    )
    repo = YFinanceOptionsRepository()
...
def test_get_options_by_moneyness_success(monkeypatch):
...
@pytest.mark.xfail(reason="pandas boolean indexing not reliably emulated in this test harness", strict=False)
def test_get_options_skew_various_errors(monkeypatch):
    # no expirations
    class T1:
        def __init__(self, t, session=None):
            self.options = []
    ...
    repo = YFinanceOptionsRepository()
    out = repo.get_options_skew("AAPL")
    assert "error" in out
    ...
    out2 = repo.get_options_skew("AAPL")
    ...
    out3 = repo.get_options_skew("AAPL")
